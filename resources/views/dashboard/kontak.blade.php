...
@section('content')
    ...
    <div class="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        <div class="bg-white rounded-2xl border border-slate-200 p-6">
            ...
            <form action="{{ route('dashboard.kontak.store') }}" method="POST" class="space-y-4">
                @csrf
                ...
                <div>
                    ...
                    <input type="text" value="{{ Auth::guard('student')->user()->nama_lengkap }}" disabled
                        class="w-full border border-slate-200 rounded-xl px-4 py-2.5 text-sm bg-slate-50 text-slate-500">
                </div>
                <div>
                    <label class="block text-sm font-semibold text-slate-700 mb-1">Email</label>
                    <input type="email" value="{{ Auth::guard('student')->user()->email }}" disabled
                        class="w-full border border-slate-200 rounded-xl px-4 py-2.5 text-sm bg-slate-50 text-slate-500">
                </div>
                <div>
                    <label class="block text-sm font-semibold text-slate-700 mb-1">Subjek <span
                ...
                <div>
                    <label class="block text-sm font-semibold text-slate-700 mb-1">Pesan <span
                            class="text-red-500">*</span></label>
                    <textarea name="pesan" rows="4" placeholder="Tulis pesan kamu..." required
                ...
                <button type="submit"
                    class="w-full bg-blue-700 text-white font-bold py-3 rounded-xl hover:bg-blue-800 transition text-sm">
                    Kirim Pesan
                </button>
            </form>
        </div>
